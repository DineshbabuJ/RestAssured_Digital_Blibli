Feature: API testing for digital
  Scenario: GET items in digital catalog and get brand id
    Given User has access to the api "http://digital-catalog.qa2-sg.cld/digital-catalog/api/"
...
#
  ...
  Scenario:To delete the existing cart for the customer
    Given User has access to the api "http://x-pulsa.qa2-sg.cld/x-pulsa/api/"
